...
class apzImageTextOverlay:
    ...
    def apz_add_text_overlay(self, image, theText, theTextbox_width, theTextbox_height, max_font_size, font, alignment, vertical_alignment, font_color, box_start_x, box_start_y, padding, line_height_ratio):
        image_tensor = image
            ...
            if apz_total_text_height <= apz_effective_textbox_height:
                ...
                for line in lines:
                    ...
                    for chunk, styles in parsed_line:
                        font_style = font
                        if styles['b']:
                            font_style = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
                        if styles['i']:
                            font_style = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Oblique.ttf"
                        if not os.path.exists(font_style):
                            font_style = default_font
                        apz_loaded_font = ImageFont.truetype(font_style, font_size)
                        draw.text((x, y), chunk, fill=font_color_rgb, font=apz_loaded_font)
                        chunk_width = apz_loaded_font.getbbox(chunk)[2] - apz_loaded_font.getbbox(chunk)[0]
                        
                        if styles['u']:
                            underline_y = y + apz_loaded_font.getsize(chunk)[1]
                            draw.line((x, underline_y, x + chunk_width, underline_y), fill=font_color_rgb, width=1)
                        if styles['s']:
                            strikeout_y = y + apz_loaded_font.getsize(chunk)[1] // 2
                            draw.line((x, strikeout_y, x + chunk_width, strikeout_y), fill=font_color_rgb, width=1)
