...
class APZmediaBrandRichTextOverlayV2:
    def __init__(self, device="cpu"):
        print("APZmediaBrandRichTextOverlayV2 initialized with enhanced error handling")
        self.device = device
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "show_error_indicators": (["false", "true"], {"default": "true"}),
                "hashtag_color": ("STRING", {"default": "#0066CC"}),
                "enable_hashtag_support": (["false", "true"], {"default": "true"}),
                "enable_emoji_support": (["false", "true"], {"default": "true"}),
                "custom_emoji_font_url": ("STRING", {"default": "", "multiline": False}),
            },
            "optional": {
    ...
    FUNCTION = "apz_add_brand_text_overlay_v2"
    CATEGORY = "APZmedia/Brand Text"

    def apz_add_brand_text_overlay_v2(self, image, brand_assets, theText, theTextbox_width, theTextbox_height, max_font_size, font_type, font_variant, alignment, vertical_alignment, font_color, italic_font_color, bold_font_color, box_start_x, box_start_y, padding, line_height_ratio, show_bounding_box, bounding_box_color, line_width, line_opacity, box_background_color, box_opacity, show_error_indicators, hashtag_color, enable_hashtag_support, enable_emoji_support, custom_emoji_font_url, font_override_regular="", font_override_italic="", font_override_bold=""):
        print(f"🔤 Raw text field received (full):")
        print(theText)
        print(f"📏 Text length: {len(theText)}")
        print(f"📏 Text bytes: {len(theText.encode('utf-8'))}")
        print(f"🔍 Text repr (first 200 chars): {repr(theText[:200])}")
        print(f"📝 APZmediaBrandRichTextOverlayV2 processing:")
        ...
        for idx, image_pil in enumerate(pil_images):
            ...
            # Find the font size and wrap the lines with enhanced error handling
            ...
            if font_size:
                # Use standard PIL rendering with emoji support
                ...
                success, fallback_font_size, fallback_text, scaling_message = error_handler.handle_font_scaling_fallback(
                    theText, theTextbox_width - 2 * padding, theTextbox_height - 2 * padding,
                    max_font_size, font_manager, line_height_ratio, "rich_text"
                )
                
                if success:
                    # Use the scaled font size and text
                    print(f"Font scaling fallback: {scaling_message}")
                    ...
                    if show_error_indicators == "true":
                        ...
                        # Final fallback - tiny text
                        fallback_font = font_manager.get_regular_font(2)
                        fallback_text = "Text overflow"
                        draw.text((box_left + padding, box_top + padding), fallback_text, 
                                 font=fallback_font, fill=font_color_rgb)
